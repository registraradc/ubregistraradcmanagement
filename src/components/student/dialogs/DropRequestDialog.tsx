...
const DropRequestDialog = ({ open, onClose, formData, onSuccess }: DropRequestDialogProps) => {
  const { user } = useAuth();
    ...
    try {
      ...
      if (existingRequest) {
        toast({
          ...
          description: 'You already have a pending or processing Drop request. Please wait for it to be completed.',
          variant: 'destructive',
        });
        setLoading(false);
      ...
      toast({
        ...
        description: 'Your drop course request has been queued.',
      });
      resetDialog();
      onSuccess();
    } catch (error: any) {
    ...
    <>
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Drop Course Request</DialogTitle>
            <DialogDescription>
              {step === 1 ? 'How many courses do you want to drop?' : 'Enter the details of courses to drop'}
            </DialogDescription>
          </DialogHeader>

          {step === 1 && (
            <div className="py-6">
              <Label className="text-center block mb-4">Number of courses to drop</Label>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to submit this drop course request? Please verify all details are correct.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default DropRequestDialog;
